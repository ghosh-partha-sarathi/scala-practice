...
RUN sudo sh -c '(echo "#!/usr/bin/env sh" && curl -L https://github.com/lihaoyi/Ammonite/releases/download/2.5.3/2.13-2.5.3) > /usr/local/bin/amm && chmod +x /usr/local/bin/amm'
